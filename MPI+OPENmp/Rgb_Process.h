...
//#pragma omp parallel for private(i,j,k,l,sum,flag) num_threads(num_of_threads)  //afairethhke giati den emfanizotan swsta h eikona
  for(i=1;i<=P->x_image_size;i++)
  {
...
void InitializeProcess_RGB(RGB_Process* P,char* filename ,int rank,int x,int y,int proc_num)
{
  ...
  P->New_Image_Array=(RGB_Pixel**)malloc((P->x_image_size+2)*sizeof(RGB_Pixel *));
#pragma omp parallel for private(i) num_threads(num_of_threads)
  	for( i=0;i<=P->x_image_size+1;i++)
  	{
